...
latest revision on the current branch. HEAD is mostly useful to specify
...
\end{verbatim}
There is also another way of specifying relative revisions by using a 
'\verb ^ '  instead of '\verb ~ '. These are equivalent, \emph{except} in the
case when more than two branches have been merged simultaneously in a so
called octopus merge (see \Ref{sec:merge}). For the exact explanation see
the \texttt{git-rev-parse} man page.
