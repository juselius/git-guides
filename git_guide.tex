...
(RADOVAN: the "let somebody pull them from you" is repeated)
...
(RADOVAN: they are deleted permanently but not irrevocably)
...
(RADOVAN: sentence is a bit strange, clear what it wants to say, but maybe rephrase)
\begin{verbatim}
$ git status
\end{verbatim}
...
Every commit (everything in fact) is internally identified by a
cryptography-strength one-way SHA1 hash (40 hexdigits), which \emph{uniquely}
identifies any commit (or file). Where ever a revision is needed one can give
