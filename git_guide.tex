...
%% I guess that more common workflow is
%% \begin{verbatim}
%% $ git diff
%% $ git commit -a
%% \end{verbatim}
%% as output of git-status is as comment in buffer when editing commit
%% message, and git-diff provides review of changes.
%%
%% Small commits matters also for commit review, and when finding bug
%% using bisect, or for git-blame analysiss.
...
%% Perhaps you should mentione here "man gitignore", and that ordering
%% of rules matters (important when excluding some files from
%% ignoring). Also fo note is fact that patterns and names match
%% basename (file name part) of filename, unless there is '/' in it.
...
%% Note: git-diff shows difference between index (staging area) and
%% working copy, not between HEAD (last commit) and working copy.
This can be
...
\end{verbatim}
%% To reset file to given state (cleaning also 'unmerged file' status)
%% one can use
%% \begin{verbatim}
%% $ git checkout HEAD~7 foobar.c
%% \end{verbatim}
%% But perhaps it is too much detail
...
changed it and when.
%% git-annotate is alias for git-blame with some backward compatibilty
%% options set; there was separate git-annotate command, but it lost
%% against git-blame.
You can also use
\begin{verbatim}
$ git gui blame file
...
$ rm *.patch
\end{verbatim}
%% --cc myself@myhost.net is not needed if you setup your identity correctly
...
%% not exactly "normal manner": use 'git am --continue', but git would
%% tell you this.
...
\end{verbatim}
%% Compression is enabled by default; what was written would _reduce_
%% compresion!
